...
echo "Deploying $DEV_REPO_NAME to $CONTAINER_NAME"
cd "$(dirname "$0")/../docker" # change to the docker directory one level up from the script's location
docker pull $DEV_REPO_NAME # pull the latest image from the repository
...
docker run -d --name $CONTAINER_NAME -p 80:80 $DEV_REPO_NAME # run the new container with the pulled image